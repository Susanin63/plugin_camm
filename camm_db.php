 ...
 */
 ...
 //***********************************************************
 	...
 	$task = '';
 	if ( isset_request_var('task')){
 		$task = get_nfilter_request_var('task');
 	}elseif( isset_request_var('task')){
 		$task = get_nfilter_request_var('task');
 	}
	...
	camm_input_validate_input_regex(get_request_var_request("rule_id"), "/^[0-9]{1,5}$/" ,'Uncorrect input data [rule_id]');
 	...
 	if ($rezult==1){
 		if ($cacti_camm_components['snmptt']) {
 			$row_start = (integer) (isset_request_var('start') ? get_nfilter_request_var('start') : 0);
 			$row_limit = (integer) (isset_request_var('limit') ? get_nfilter_request_var('limit') : read_config_option("camm_num_rows"));
 			$tree_id = (integer) (isset_request_var('tree_id') ? get_nfilter_request_var('tree_id') : 0);
 			$rule_id = (integer) (isset_request_var('rule_id') ? get_nfilter_request_var('rule_id') : 0);
			$raw_json_where = (string) (isset_request_var('filter') ? get_nfilter_request_var('filter') : '');
			$raw_json_where_chart = (string) (isset_request_var('filter_chart') ? get_nfilter_request_var('filter_chart') : '');
			...
			//todo change to 
			// $sql_where .= (strlen($sql_where) ? ' AND ': 'WHERE ') .  camm_create_sql_from_period($tree_node['period'],$tree_node['type']);
			if (isset($tree_node['period']) and isset($tree_node['type'])) {
			...
			if ($rule_id == 0) {
				if (($tree_id > 0) and (isset($tree_node['_is_marker']) and $tree_node['_is_marker'] == 1)) {
					if ($tree_node['gr_v'] > 0) {
 ...
 $rezult=1;
 	...
 	if ($rezult==1){
 		if ($cacti_camm_components['snmptt']) {
 			...
 			$row_start = (integer) (isset_request_var('start') ? get_nfilter_request_var('start') : get_nfilter_request_var('start'));
 			$row_limit = (integer) (isset_request_var('limit') ? get_nfilter_request_var('limit') : read_config_option("camm_num_rows"));
 			$raw_json_where = (string) (isset_request_var('filter') ? get_nfilter_request_var('filter') : get_nfilter_request_var('filter'));
...
$rezult=1;
 	...
 	if ($rezult==1){
	   ...
	   $row_start = (integer) (isset_request_var('start') ? get_nfilter_request_var('start') : "0");
	   $row_limit = (integer) (isset_request_var('limit') ? get_nfilter_request_var('limit') : "50");
	   $raw_json_where = (string) (isset_request_var('filter') ? get_nfilter_request_var('filter') : get_nfilter_request_var('filter'));
 	...
 	if ($rezult==1){
 		if ($cacti_camm_components['snmptt']) {
 			if ( isset_request_var('id')){
				if (isset_request_var('dir')) {
					$dir = get_nfilter_request_var('dir'); // Get 
				}else {
					$dir = ""; // Get 
				}			
 			   $id = get_nfilter_request_var('id'); // Get our array back and translate it :
 ...
 $rezult=1;
	...
	camm_input_validate_input_regex(stripslashes(get_request_var_request("stats")), "/^(0|1)$/", 'Incorrect value [stats]');
 	...
 	$type_tree = (string) (isset_request_var('type') ? get_nfilter_request_var('type') : "snmptt");
	$stats = (string) (isset_request_var('stats') ? get_nfilter_request_var('stats') : "0");
...
$rezult=1;
	...
	camm_input_validate_input_regex(stripslashes(get_request_var_request("period")), "/^(all|day|week|hour)$/");	
 	...
 	//business logic
 	if ($rezult==1){
	$type_tree = (string) strtolower(isset_request_var('type') ? get_nfilter_request_var('type') : 'Syslog');
	$period = (string) strtolower(isset_request_var('period') ? get_nfilter_request_var('period') : 'all');
...
$rezult=1;
 	...
 	if ($rezult==1){
		...
		$row_start = (integer) (isset_request_var('start') ? get_nfilter_request_var('start') : get_nfilter_request_var('start'));
		$row_limit = (integer) (isset_request_var('limit') ? get_nfilter_request_var('limit') : get_nfilter_request_var('limit'));
...
camm_input_validate_input_regex(get_request_var_request("interval", "300"), "/^[0-9]{1,5}$/");
	...
	$type_tree = (string) strtolower(isset_request_var('type') ? get_nfilter_request_var('type') : 'syslog');
	$rule_id = (integer) strtolower(isset_request_var('rule_id') ? get_nfilter_request_var('rule_id') : '0');
	$period = (integer) strtolower(isset_request_var('period') ? get_nfilter_request_var('period') : '0');
	$point_interval = (integer) strtolower(isset_request_var('interval') ? get_nfilter_request_var('interval') : '300');
	...
	//one more business check
 	if ($rezult==1){
		...
		if (sizeof($rows) > 0) {
			...
			// Create a varaible holding the array
			$point_end_key_array = array_keys($rows);
			// 'end' takes a reference to an array, which needs to be a variable
			$point_end_key = end($point_end_key_array);
			
			$point_end=$rows[$point_end_key]['time'];
...
//camm_input_validate_input_regex(get_request_var_request("interval", "300"), "/^[0-9]{1,5}$/");
	...
	$type_tree = (string) strtolower(isset_request_var('type') ? get_nfilter_request_var('type') : 'syslog');
	$rule_id = (integer) strtolower(isset_request_var('rule_id') ? get_nfilter_request_var('rule_id') : '0');
	$limit = (integer) (isset_request_var('limit') ? get_nfilter_request_var('limit') : '10');
	$raw_json_where_chart = (string) (isset_request_var('filter_by_chart') ? get_nfilter_request_var('filter_by_chart') : '');
	$use_cacti = (string) (isset_request_var('use_cacti') ? get_nfilter_request_var('use_cacti') : true);
	$use_host_group = (string) (isset_request_var('use_host_group') ? get_nfilter_request_var('use_host_group') : true);
	// $point_interval = (integer) strtolower(isset_request_var('interval') ? get_nfilter_request_var('interval') : '300');
