...
if (!defined('SNMP_METHOD_PHP_SET')) {
    define("SNMP_METHOD_PHP_SET", 1);
}

if (!defined('SNMP_METHOD_BINARY_SET')) {
...
if (!defined('SNMP_SET_ESCAPE_CHARACTER')) {
 		...
 		foreach ($arr as $tree_type) {
			...
			foreach ($periods as $period) {
				...
				if ($tree_type == 'snmptt') {
					...
					//db_execute("update plugin_camm_tree2 set _parent='2' where _is_marker=1 and  _lvl=2;");
...
if ($tree_id > 0) {
	...
	if (is_array($tree_row) and isset($tree_row['type'])) {
...
function camm_top_graph_header() {
	global $config;

	if (!isset_request_var('header') || get_nfilter_request_var('header') == 'true') {
		include_once($config['base_path'] . '/plugins/camm/camm_top_general_header.php');
	}
}
...
/* html_common_header - prints a common set of header, css and javascript links
   ...
   @arg selectedTheme - optionally sets a specific theme over the current one
*/
function camm_html_common_header($title, $selectedTheme = '') {
	...
	//do NOT load pace - not work with ExtJS
	...
	//do NOT load layout.js - not work with ExtJS
	//print get_md5_include_js('include/layout.js');
	//do NOT load pace.js - not work with ExtJS
